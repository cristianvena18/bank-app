...
import { DomainEvent } from "../../../Domain/Interfaces/EventBus/DomainEvent";
import { DomainEventSubscriber } from "../../../Domain/Interfaces/EventBus/DomainEventSubscriber";
...
export default class RabbitMqEventbus implements EventBus {
  ...
  constructor(
    @inject(INTERFACES.RabbitMqConfig) config: RabbitMqConfig,
    @inject(INTERFACES.LoggerService) logger: LoggerService
  ) {
    this.logger = logger;
